...
- **Never guess a flag name, and never invent one.** Several subcommands take bare positionals — `screenshot [selector] [path]` is the common one — and the underlying CLI accepts an unrecognized flag as a positional instead of rejecting it. So `screenshot --full-page` writes an image to a file literally named `--full-page` in the working directory and reports success, leaving junk in the user's repo. The two that get guessed wrong most: it is `--full` (not `--full-page`) and `--screenshot-format` (not `--format`). The wrapper now refuses unknown flags and prints the valid set, so a wrong guess is a loud error rather than a stray file — but run `agent-browser <subcommand> --help` when unsure rather than relying on the guard, and pass an output path positionally (`screenshot ./shot.png`) rather than reaching for a flag.
